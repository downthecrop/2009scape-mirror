...
class DukeHoracioDSDialogue(val questStage: Int) : DialogueFile() {
    ...
    fun hasShield(player: Player): Boolean{
        return player.inventory.containsItem(DragonSlayer.SHIELD) || player.bank.containsItem(DragonSlayer.SHIELD) || player.equipment.containsItem(DragonSlayer.SHIELD)
    }
}