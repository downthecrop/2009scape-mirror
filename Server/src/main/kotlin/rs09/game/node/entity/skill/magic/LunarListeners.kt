...
class LunarListeners : SpellListener("lunar") {
    ...
    private fun superglassMake(player: Player) {
        ...
        var playerWeed: Int = 0
        var playerSand: Int = 0
        var index: Int = 0

        for(item in inv) {
            if(item == null) continue
            if(GLASSWEED.contains(item.id)) playerWeed++
            if(item.id == Items.BUCKET_OF_SAND_1783) playerSand++
        }
        ...
        for(item in inv) {
            if(item == null) continue
            if(index == size) break
            if(playerWeed == 0 || playerSand == 0) { sendMessage(player, "You lack the required ingredients."); break }
