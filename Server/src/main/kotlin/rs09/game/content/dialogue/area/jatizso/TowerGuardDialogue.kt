...
class TowerGuardDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage){
            ...
            4 -> npc(FacialExpression.NEUTRAL, "THE TOWER IN NEITIZNOT.","THEY SHOUT INSULTS BACK.").also { stage++ }
            5 -> playerl(FacialExpression.ASKING, "Err, why?" ).also { stage++ }
            6 -> npc(FacialExpression.NEUTRAL, "THE KING TELLS US TO.").also { stage++ }
            7 -> playerl(FacialExpression.HALF_THINKING, "Your King is a strange person.").also { stage++ }
