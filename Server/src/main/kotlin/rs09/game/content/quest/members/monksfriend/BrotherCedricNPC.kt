        ...
        when {
            ...
            questStage == 42 -> {
                when(stage){
                    ...
                    4 -> playerl(FacialExpression.HAPPY,"Ok! I'll see you later!").also{stage=END_DIALOGUE;player!!.inventory.remove(Item(Items.LOGS_1511))
                        setQuest(player!!, 50)}
                }
            }
            questStage == 50 -> {
...
class BrotherCedricListener : InteractionListener() {
    override fun defineListeners() {
        on(NPCs.BROTHER_CEDRIC_280, NPC, "talk-to"){ player, node ->
