...
class SeersHouseListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(WESTLADDER, IntType.SCENERY, "Climb-up") { player, node ->
            core.game.global.action.ClimbActionHandler.climb(player, Animation(828), Location.create(2631, 3664, 2))
            return@on true
        }

        on(WESTTRAPDOOR.id, IntType.SCENERY, "Climb-down") { player, node ->
            if(player.location.x < 2634){
                core.game.global.action.ClimbActionHandler.climb(player, Animation(828), Location.create(2631, 3664, 0))
            }
            else if(player.location.x > 2634){
                core.game.global.action.ClimbActionHandler.climb(player, Animation(828), Location.create(2636, 3664, 0))
            }
            return@on true
        }
        ...
        on(EASTLADDER, IntType.SCENERY, "Climb-Up") { player, node ->
            core.game.global.action.ClimbActionHandler.climb(player, Animation(828), Location.create(2636, 3664, 2))
            return@on true
        }
