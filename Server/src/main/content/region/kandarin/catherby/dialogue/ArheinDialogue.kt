...
@Initializable
class ArheinDialogue(player: Player? = null) : core.game.dialogue.DialoguePlugin(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if (args.size > 1) {
            npcl(core.game.dialogue.FacialExpression.ANGRY, "Hey buddy! Get away from my ship alright?")
            stage = 701
        } else {
            npcl(core.game.dialogue.FacialExpression.HAPPY, "Hello! Would you like to trade? I've a variety of wares for sale!")
            stage = 1
        }
        return true
    }
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage) {
            ...
            100 -> npcl(core.game.dialogue.FacialExpression.NEUTRAL, "Yes, I use it to make deliveries to my customers up and down the coast. These crates here are all ready for my next trip.").also { stage++ }
            101 -> showTopics(
                Topic("Where do you deliver to?",120),
                Topic("Are you rich then?", 110),
                Topic("Do you deliver to the fort just down the coast?",500)
            )
            110 -> npcl(core.game.dialogue.FacialExpression.NEUTRAL, "Business is going reasonably well... I wouldn't say I was the richest of merchants ever, but I'm doing fairly well all things considered.").also { stage = END_DIALOGUE }
            120 -> npcl(core.game.dialogue.FacialExpression.NEUTRAL, "Various places up and down the coast. Mostly Karamja and Port Sarim.").also { stage++ }
            121 -> showTopics(
                Topic("I don't suppose I could get a lift anywhere?",140),
                Topic("Well, good luck with your business.",130)
            )
            130 -> npcl(core.game.dialogue.FacialExpression.HAPPY,"Thanks buddy!").also { stage = END_DIALOGUE }
            ...
            701 -> playerl(core.game.dialogue.FacialExpression.GUILTY,"Yeah... uh... sorry...").also{ stage = END_DIALOGUE }
            800 -> {
                // Pineapples
                selectGoods(Items.PINEAPPLE_2114)
                // and now the conditional block to handle cases
                if(stock==0) {
                    npcl(core.game.dialogue.FacialExpression.SAD,"Actually, I've run out. Come back tomorrow and I should have some more.")
                    stage = END_DIALOGUE
                } else {
                    ...
                    npcl(core.game.dialogue.FacialExpression.HAPPY,this.goodsMessage)
            ...
            1200 -> {
                ...
                sendInputDialogue(player,InputType.AMOUNT, goodsPrompt) { value ->
                    ...
                    npcl(core.game.dialogue.FacialExpression.HAPPY,exitMsg)
                }
                stage = END_DIALOGUE
            }
        }
